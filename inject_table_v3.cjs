...
const tableCode = `                             {/* Balance History Table */}
...
if (!c.includes('Detailed History')) {
    // We want to insert after the end of Balance History Chart container
    // The Balance History Chart is under `{/* Balance History Chart */}`
    const startIdx = c.indexOf('{/* Balance History Chart */}');
    if (startIdx !== -1) {
        // Find the end of this div block. It's inside a flex container or something.
        // Actually, we can just replace the specific end pattern for that section.
        // The pattern is:
        //                                      )}
        //                                  </div>
        //                              </div>
        ...
        // Let's match the closing tags right before {/* --- MODALS --- */}
        const regex = /(<\/div>\s*<\/div>\s*<\/div>\s*\)\}\s*<\/div>\s*\{\/\* --- MODALS --- \*\/)/;
        
        if (regex.test(c)) {
            c = c.replace(regex, tableCode + '\n$1');
            fs.writeFileSync(filePath, c);
            console.log("Table inserted successfully with regex!");
        } else {
            console.log("Could not find the closing tags regex pattern.");
        }
    } else {
        console.log("Could not find Balance History Chart");
    }
} else {
