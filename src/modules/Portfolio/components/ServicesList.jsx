        ...
        <div className={`space-y-6 ${isSectionHidden ? 'opacity-50' : ''}`}>
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <h3 className="text-sm font-black uppercase tracking-widest text-slate-500">🛠️ Services</h3>
                ...
                {viewMode === 'admin' && (
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            ...
                            className="p-2 bg-blue-600/20 text-blue-500 rounded-lg hover:bg-blue-600 hover:text-blue-600 transition-all"
                        >
            ...
            {/* Add Form */}
            {isAdding && (
                <div className="p-4 bg-white shadow-sm border border-slate-200/50 border border-blue-500/30 rounded-xl space-y-3 animate-in fade-in slide-in-from-top-2">
                    <input
                    ...
                    <div className="flex gap-2">
                        <input
                            ...
                            className="flex-1 bg-white shadow-sm border border-slate-300 rounded-lg p-2 text-xs text-slate-800 focus:border-blue-500/50 outline-none"
                    ...
                    <div className="flex gap-2">
                        ...
                        <input
                            ...
                            className="w-1/3 bg-white shadow-sm border border-slate-300 rounded-lg p-2 text-xs text-slate-800 focus:border-blue-500/50 outline-none"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {services.map(service => (
                    <div key={service.id} className={`group relative p-5 bg-white shadow-sm border border-slate-200/30 border rounded-2xl transition-all flex flex-col h-full ${service.isHidden ? 'border-red-900/30 opacity-60 hover:opacity-100' : 'border-slate-200 hover:border-slate-300'}`}>
                        {editingId === service.id ? (
                            <div className="space-y-3 z-10 relative flex-1">
                                <input
                                    className="w-full bg-white shadow-sm border border-blue-500/30 rounded p-1 text-sm font-bold text-slate-800"
                                    value={editData.title}
                                    onChange={e => setEditData({ ...editData, title: e.target.value })}
                                />
                                <div className="flex gap-2">
                                    <input
                                        className="flex-1 bg-white shadow-sm border border-blue-500/30 rounded p-1 text-xs text-amber-600"
                                        value={editData.price}
                                        onChange={e => setEditData({ ...editData, price: e.target.value })}
                                    />
                                    <input
                                        className="flex-1 bg-white shadow-sm border border-blue-500/30 rounded p-1 text-xs text-slate-500"
                                ...
                                <div className="flex gap-2">
                                    <input
                                        className="flex-1 bg-white shadow-sm border border-blue-500/30 rounded p-1 text-xs text-blue-400"
                                        placeholder="Link URL..."
                                        value={editData.link || ''}
                                        onChange={e => setEditData({ ...editData, link: e.target.value })}
                                    />
                                    <input
                                        className="w-1/3 bg-white shadow-sm border border-blue-500/30 rounded p-1 text-xs text-blue-400"
                                ...
                                <textarea
                                    className="w-full bg-white shadow-sm border border-blue-500/30 rounded p-1 text-xs text-slate-600 min-h-[60px]"
                            ...
                            <>
                                <div className="flex-1">
                                    <div className="flex justify-between items-start mb-2">
                                        <h4 className="font-bold text-base text-slate-700 pr-6">{service.title}</h4>
                                        <div className="text-right">
                                            <div className="text-xs font-bold text-amber-600 bg-yellow-900/20 px-2 py-1 rounded border border-slate-200">{service.price}</div>
                                            <div className="text-[10px] text-slate-500 font-mono mt-1 uppercase">{service.timeframe}</div>
                                        </div>
                                    </div>
                                ...
                                {viewMode === 'admin' && (
                                    <div className="opacity-0 group-hover:opacity-100 transition-opacity absolute top-4 right-4 flex gap-1 bg-white/80 rounded p-1">
                                        <button onClick={() => startEdit(service)} className="text-slate-500 hover:text-blue-500 p-1"><Edit2 className="w-3 h-3" /></button>
                                        <button onClick={() => actions.delete(service.id)} className="text-slate-500 hover:text-red-500 p-1"><Trash2 className="w-3 h-3" /></button>
                                    </div>
                                )}
                            </>
                        )}
