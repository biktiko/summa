        ...
        <div className={`space-y-6`}>
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-sm font-black uppercase tracking-widest text-slate-600">Project Forge</h3>
            ...
            {isAdding && (
                <div className="p-4 bg-white shadow-sm border border-slate-200/50 border border-blue-500/30 rounded-xl space-y-4 animate-in fade-in slide-in-from-top-2">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="space-y-1">
                            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider ml-1">Impact</label>
                            <input
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                            ...
                            <input
                                ...
                                className="w-full bg-white shadow-sm border border-slate-300 rounded-lg p-2 text-xs text-blue-400 focus:border-blue-500/50 outline-none"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {projects.map(project => (
                    <div key={project.id} className={`group relative p-6 bg-white shadow-sm border border-slate-200/30 border rounded-2xl transition-all flex flex-col h-full ${project.isHidden ? 'border-red-900/30 opacity-60 hover:opacity-100' : 'border-slate-200 hover:border-blue-500/20'}`}>
                            ...
                            <div className="space-y-4 z-10 relative flex-1">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1">
                                        <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider ml-1">Status</label>
                                        <input
                                            className="w-full bg-white shadow-sm border border-blue-500/30 rounded p-2 text-xs text-amber-600 outline-none focus:border-blue-500"
                                    ...
                                    <div className="space-y-1">
                                        <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider ml-1">Impact</label>
                                        <input
                                            className="w-full bg-white shadow-sm border border-blue-500/30 rounded p-2 text-xs text-green-500 outline-none focus:border-blue-500"
                            ...
                            <>
                                <div className="flex-1">
                                    <div className="flex justify-between items-start mb-2">
                                        <h4 className="font-bold text-base text-slate-700 pr-6">{project.name}</h4>
                                ...
                                {viewMode === 'admin' && (
                                    <div className="opacity-0 group-hover:opacity-100 transition-opacity absolute top-4 right-4 flex gap-1 bg-white/80 rounded p-1">
                                        <button onClick={() => startEdit(project)} className="text-slate-500 hover:text-blue-500 p-1"><Edit2 className="w-3 h-3" /></button>
                                        <button onClick={() => actions.delete(project.id)} className="text-slate-500 hover:text-red-500 p-1"><Trash2 className="w-3 h-3" /></button>
                                    </div>
                                )}
                            </>
                        )}
