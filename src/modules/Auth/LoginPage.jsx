...
const LoginPage = () => {
    ...
    return (
        <div className="min-h-screen bg-[#020202] flex items-center justify-center p-4 font-sans text-neutral-200 relative overflow-hidden">
            ...
            <div className="w-full max-w-md bg-neutral-900/80 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl relative z-10 hidden-scrollbar overflow-y-auto max-h-[95vh]">
                ...
                <div className="flex bg-black/40 p-1 rounded-xl mb-6 border border-white/5">
                    <button 
                        ...
                        className={`flex-1 py-2 text-xs font-black uppercase tracking-widest rounded-lg transition-all ${isLogin ? 'bg-neutral-800 text-white shadow-lg' : 'text-neutral-500 hover:text-neutral-300'}`}
                    >
                        Login
                    </button>
                    <button 
                         onClick={() => { setIsLogin(false); setError(''); }}
                         className={`flex-1 py-2 text-xs font-black uppercase tracking-widest rounded-lg transition-all ${!isLogin ? 'bg-blue-900/50 text-blue-200 border border-blue-500/30' : 'text-neutral-500 hover:text-neutral-300'}`}
                    >
                        Register
                    </button>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    {!isLogin && (
                        <div className="relative group animate-in slide-in-from-top-2 fade-in">
                            ...
                            <input
                                ...
                                className="w-full bg-black/40 border border-white/10 rounded-xl py-3 pl-10 pr-4 text-sm outline-none focus:border-blue-500/50 transition-all placeholder:text-neutral-600"
                                required
                            />
                        </div>
                    )}
                    ...
                    <button
                        type="submit"
                        disabled={isLoading}
                        className={`w-full text-white font-black uppercase tracking-widest py-3.5 rounded-xl shadow-lg flex items-center justify-center gap-2 transition-all mt-4 disabled:opacity-50 disabled:cursor-not-allowed ${isLogin ? 'bg-blue-600 hover:bg-blue-500 shadow-blue-900/20' : 'bg-indigo-600 hover:bg-indigo-500 shadow-indigo-900/20'}`}
                    >
