...
const BiometricScanner = ({ biometrics, actions, viewMode, isSectionHidden, toggleSectionVisibility }) => {
    ...
    const getStatus = (val, optMin, optMax, critMin, critMax) => {
        ...
        return { color: 'text-yellow-500', bg: 'bg-yellow-500', border: 'border-yellow-500', label: 'WARNING' };
    };
    ...
    return (
        <div className={`h-full flex flex-col ${isSectionHidden ? 'opacity-50 grayscale' : ''}`}>
            ...
            <div className="flex items-center justify-between mb-8">
                <div>
                    <h2 className="text-3xl font-black uppercase tracking-tighter text-white mb-1">Bio-Telemetry {isSectionHidden && <span className="text-xs text-red-500 ml-2">(HIDDEN)</span>}</h2>
                    <p className="text-xs text-neutral-500 font-mono uppercase tracking-widest">Comprehensive Health Diagnostics</p>
                </div>
                {viewMode === 'admin' && (
            ...
            <div className="space-y-12 pb-20">
                {Object.keys(groupedBiometrics).length === 0 && !isAdding && (
                    <div className="flex flex-col items-center justify-center py-20 border border-dashed border-white/5 rounded-3xl bg-neutral-900/20">
                        <Activity className="w-12 h-12 text-neutral-700 mb-4" />
                        <p className="text-neutral-500 text-sm uppercase tracking-widest">System Offline: No Data</p>
                    </div>
                )}
                ...
                {Object.entries(groupedBiometrics).map(([category, items]) => (
                    <div key={category} className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="flex items-center gap-3 mb-6 border-b border-white/5 pb-2">
                            ...
                            <span className="text-[10px] font-mono text-neutral-600 bg-white/5 px-2 py-0.5 rounded-full">{items.length}</span>
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            {items.map(item => {
                                ...
                                return (
                                    <div
                                        key={item.id}
                                        className={`bg-neutral-900/40 border ${status.border}/20 rounded-2xl overflow-hidden transition-all hover:border-${status.color.split('-')[1]}-500/50 group`}
                                    >
                                        {/* Main Row */}
                                        <div className="p-6 flex flex-col md:flex-row md:items-center gap-6">
                                            ...
                                            <div className="w-full md:w-1/4">
                                                <div className="flex items-center gap-2 mb-1">
                                                    ...
                                                    {item.description && <Info className="w-3 h-3 text-neutral-600" />}
                                                </div>
                                                <div className="flex items-baseline gap-2">
                                                    <span className={`text-3xl font-black tracking-tighter ${status.color}`}>
                                                        {item.value}
                                                    </span>
                                                    <span className="text-xs text-neutral-500 font-mono font-bold">{item.unit}</span>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex-1 relative h-12 flex items-center">
                                                ...
                                                {/* Labels underneath */}
                                                <div className="absolute -bottom-1 w-full flex justify-between text-[9px] text-neutral-600 font-mono uppercase">
                                            ...
                                            <div className="w-full md:w-auto flex items-center justify-between md:justify-end gap-4">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {viewMode === 'admin' && (
                                                        <>
                                                            <button onClick={() => startEdit(item)} className="p-2 hover:bg-white/5 rounded-lg text-neutral-500 hover:text-white transition-colors">
                                                                <Edit2 className="w-4 h-4" />
                                                            </button>
                                                            <button onClick={() => handleDelete(item.id)} className="p-2 hover:bg-red-500/10 rounded-lg text-neutral-500 hover:text-red-500 transition-colors">
                                        ...
                                        {/* Expanded Details */}
                                        {isExpanded && (
                                            <div className="px-6 pb-6 pt-0 border-t border-white/5 mt-2 animate-in slide-in-from-top-2">
                                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-4">
                                                    <div className="bg-black/20 p-4 rounded-xl border border-white/5">
                                                        <h5 className="text-[10px] font-bold text-neutral-500 uppercase mb-2">Analysis</h5>
                                                        <p className="text-sm text-neutral-300 leading-relaxed">
                                                            {item.description || "No detailed analysis provided for this metric."}
                                                        </p>
                                                    </div>
                                                    <div className="bg-black/20 p-4 rounded-xl border border-white/5">
                                                        <h5 className="text-[10px] font-bold text-neutral-500 uppercase mb-2">Target Strategy</h5>
                                                        <div className="flex items-center justify-between mb-2">
                                                            <span className="text-xs text-neutral-400">Goal Value</span>
                                                            <span className="text-sm font-bold text-blue-400">{item.target || '--'} {item.unit}</span>
                                                        </div>
                                                        <div className="flex items-center justify-between">
                                                            <span className="text-xs text-neutral-400">Gap to Target</span>
                                                    ...
                                                    <div className="bg-black/20 p-4 rounded-xl border border-white/5">
                                                        <h5 className="text-[10px] font-bold text-neutral-500 uppercase mb-2">Reference Ranges</h5>
                                                        <div className="space-y-1">
                                                            <div className="flex justify-between text-xs"><span className="text-emerald-500">Optimal</span> <span className="text-neutral-300">{item.optimalMin} - {item.optimalMax}</span></div>
                                                            <div className="flex justify-between text-xs"><span className="text-yellow-500">Warning</span> <span className="text-neutral-300">&lt; {item.optimalMin} or &gt; {item.optimalMax}</span></div>
                                                            <div className="flex justify-between text-xs"><span className="text-red-500">Critical</span> <span className="text-neutral-300">&lt; {item.criticalMin} or &gt; {item.criticalMax}</span></div>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                {item.history && item.history.length > 1 && (
                                                    <div className="mt-6 bg-black/20 p-4 rounded-xl border border-white/5">
                                                        <h5 className="text-[10px] font-bold text-neutral-500 uppercase mb-4">Historical Trend</h5>
                                                                                ...
                                                                                <g key={i} className="group/point">
                                                                                    ...
                                                                                    <foreignObject x={`${x}%`} y={`${y}%`} width="100" height="100" className="overflow-visible">
                                                                                        <div className="transform -translate-x-1/2 -translate-y-full pb-2 opacity-0 group-hover/point:opacity-100 transition-opacity">
                                                                                            <div className="bg-neutral-900 text-[10px] p-1 rounded border border-white/10 whitespace-nowrap">
                                                                                                <div className="font-bold text-white">{d.value} {item.unit}</div>
                                                                                                <div className="text-neutral-500">{d.date}</div>
                                                                                            </div>
                                                                                        </div>
                                                                                    </foreignObject>
                                                                                </g>
                                                ...
                                                <div className="mt-4 text-[10px] text-neutral-600 font-mono text-right">
            ...
            {isAdding && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in fade-in duration-200">
                    <div className="w-full max-w-2xl bg-[#0A0A0A] border border-white/10 rounded-3xl shadow-2xl p-8 max-h-[90vh] overflow-y-auto custom-scrollbar">
                        <div className="flex justify-between items-center mb-8">
                            <h3 className="text-xl font-black text-white uppercase tracking-wider">{editingId ? 'Edit Metric' : 'New Metric'}</h3>
                            <button onClick={resetForm} className="text-neutral-500 hover:text-white transition-colors"><X className="w-6 h-6" /></button>
                        </div>
                        <form onSubmit={handleSave} className="space-y-8">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-6">
                                    <div className="col-span-2 md:col-span-1">
                                        <label className="text-[10px] font-bold text-neutral-500 uppercase block mb-2">Metric Name</label>
                                        <input
                                    ...
                                    <div className="col-span-2 md:col-span-1">
                                        <label className="text-[10px] font-bold text-neutral-500 uppercase block mb-2">Category</label>
                                        <select
                                            className="w-full bg-neutral-900 border border-white/10 rounded-xl p-3 text-sm text-white outline-none focus:border-blue-500 transition-colors appearance-none"
                                    ...
                                    <div>
                                        <label className="text-[10px] font-bold text-neutral-500 uppercase block mb-2">Current Value</label>
                                        <input
                                            type="number" step="0.01"
                                            className="w-full bg-neutral-900 border border-white/10 rounded-xl p-3 text-sm text-white outline-none focus:border-blue-500 transition-colors font-mono"
                                            value={formData.value}
                                            onChange={e => setFormData({ ...formData, value: e.target.value })}
                                        />
                                    </div>
                                    <div>
                                        <label className="text-[10px] font-bold text-neutral-500 uppercase block mb-2">Unit</label>
                                        <input
                                            className="w-full bg-neutral-900 border border-white/10 rounded-xl p-3 text-sm text-white outline-none focus:border-blue-500 transition-colors font-mono"
                                            value={formData.unit}
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    <div>
                                        ...
                                        <input
                                            type="number" step="0.01"
                                            className="w-full bg-neutral-900 border border-white/10 rounded-xl p-3 text-sm text-white outline-none focus:border-red-500 transition-colors font-mono"
                                    ...
                                    <div>
                                        ...
                                        <input
                                            type="number" step="0.01"
                                            className="w-full bg-neutral-900 border border-white/10 rounded-xl p-3 text-sm text-white outline-none focus:border-emerald-500 transition-colors font-mono"
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-1 gap-6">
                                    <div>
                                        <label className="text-[10px] font-bold text-neutral-500 uppercase block mb-2">Target Goal (Optional)</label>
                                        <input
                                            type="number" step="0.01"
                                            className="w-full bg-neutral-900 border border-white/10 rounded-xl p-3 text-sm text-white outline-none focus:border-purple-500 transition-colors font-mono"
                                    ...
                                    <div>
                                        <label className="text-[10px] font-bold text-neutral-500 uppercase block mb-2">Description / Notes</label>
                                        <textarea
                                            className="w-full bg-neutral-900 border border-white/10 rounded-xl p-3 text-sm text-white outline-none focus:border-purple-500 transition-colors min-h-[100px]"
