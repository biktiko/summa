        ...
        <div className={`group bg-white shadow-sm border border-slate-200 rounded-xl hover:border-blue-500/30 transition-all relative ${task.isHidden ? 'opacity-60 border-red-900/30' : ''} ${displayMode === 'list' && editingId !== task.id ? 'p-1.5 md:p-2' : 'p-2 md:p-4'}`}>
            {editingId === task.id ? (
                <>
                    ...
                    <div className="space-y-4 z-20 relative bg-white shadow-xl border border-slate-200 p-4 rounded-lg border border-blue-500/30 w-full max-h-[75vh] overflow-y-auto custom-scrollbar shadow-2xl" onClick={e => e.stopPropagation()}>
                    <div className="bg-white shadow-xl border border-slate-200 pb-2 border-b border-slate-200 mb-4 flex justify-between items-center">
                    ...
                    <div className="space-y-3">
                        <div className="space-y-1">
                            ...
                            <select
                                className="w-full bg-white shadow-sm border border-blue-500/30 rounded px-2 text-[11px] text-slate-800 outline-none focus:border-blue-500 h-9"
                        ...
                        <div className="grid grid-cols-2 gap-3 mt-3">
                            <div className="space-y-1">
                                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest ml-1 block">Priority</label>
                                <select
                                    className="w-full bg-white shadow-sm border border-blue-500/30 rounded px-2 text-[11px] text-slate-800 outline-none focus:border-blue-500 h-9"
                            ...
                            <div className="space-y-1">
                                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest ml-1 block">Difficulty</label>
                                <select
                                    className="w-full bg-white shadow-sm border border-blue-500/30 rounded px-2 text-[11px] text-slate-800 outline-none focus:border-blue-500 h-9"
                        ...
                        <div className="space-y-1 mt-3">
                            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest ml-1">Task Deadline</label>
                            <input
                                type="date"
                                className="w-full bg-white shadow-sm border border-blue-500/30 rounded px-3 text-[11px] text-slate-800 outline-none focus:border-blue-500 h-9"
                    ...
                    <div className="space-y-2 border-t border-slate-200 pt-2">
                        <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider ml-1">Tags</label>
                        ...
                        <div className="flex gap-2">
                            <input
                                className="flex-1 bg-white shadow-sm border border-blue-500/30 rounded p-2 text-xs text-slate-800 outline-none focus:border-blue-500"
                                placeholder="New Tag"
                            ...
                            />
                            <select
                                className="w-24 bg-white shadow-sm border border-blue-500/30 rounded p-2 text-xs text-slate-800 outline-none focus:border-blue-500"
                            ...
                            <button onClick={addTag} className="p-2 bg-blue-500/20 text-blue-500 rounded hover:bg-blue-500 hover:text-blue-600 transition-all"><Plus className="w-3 h-3" /></button>
                        </div>
                        {/* Suggested Tags (Quick Add for Edit) */}
                        {suggestedTags && suggestedTags.length > 0 && (
                            <div className="flex flex-wrap gap-1 mt-2 p-2 bg-slate-50 rounded-lg">
                                <span className="text-[9px] text-slate-500 uppercase font-bold w-full mb-1">Quick Add:</span>
                                {suggestedTags.filter(t => !(editData.tags || []).some(nt => nt.text.toLowerCase() === t.text.toLowerCase())).map(tag => (
                                    <button
                                        ...
                                        className={`text-[9px] px-2 py-1 rounded border border-slate-300 hover:border-white/30 transition-all ${tag.color} opacity-80 hover:opacity-100 pr-2`}
                                    >
                                        {tag.text}
                    ...
                    {/* Subtasks Edit */}
                    <div className="space-y-2 border-t border-slate-200 pt-2">
                        <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider ml-1">Subtasks</label>
                        <div className="space-y-2">
                            {(editData.subtasks || []).map((subtask, idx) => (
                                <div key={subtask.id || idx} className="flex items-center gap-2">
                                     <button
                                        ...
                                        className={`w-4 h-4 rounded-full border flex items-center justify-center flex-shrink-0 ${subtask.completed ? 'bg-blue-500 border-blue-500' : 'border-slate-300'}`}
                                     >
                                         {subtask.completed && <Check className="w-2.5 h-2.5 text-black" />}
                                     </button>
                                     <input
                                        className="flex-1 bg-transparent text-xs text-slate-800 outline-none border-b border-slate-300 focus:border-blue-500/50 pb-0.5"
                                        value={subtask.text}
                                        onChange={(e) => {
                                            const newSubtasks = [...(editData.subtasks || [])];
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button
                                    ...
                                    className="p-2 bg-blue-500/20 text-blue-500 rounded hover:bg-blue-500 hover:text-blue-600 transition-all"
                                >
                    ...
                    {/* Progress Fields */}
                    <div className="space-y-1">
                        <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider ml-1">Progress Tracking</label>
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                             <input
                                type="number"
                                className="w-full bg-white shadow-sm border border-blue-500/30 rounded p-2 text-xs text-slate-800 outline-none focus:border-blue-500"
                                placeholder="Current"
                    ...
                    <div className="space-y-1">
                        ...
                        <textarea
                            className="w-full bg-white shadow-sm border border-blue-500/30 rounded p-2 text-xs text-slate-600 min-h-[80px] outline-none focus:border-blue-500"
                    ...
                    <div className="flex justify-between items-center pt-2 border-t border-slate-200 mt-2">
                        <button
                            ...
                            className={`p-2 rounded flex items-center gap-2 text-xs font-bold transition-all ${editData.isHidden ? 'text-red-500 bg-red-900/20 shadow-inner' : 'text-slate-500 hover:text-blue-600 hover:bg-slate-100'}`}
                            title="Toggle Visibility"
                        >
                            {editData.isHidden ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                        </button>
                        <div className="flex gap-2">
                            <button onClick={() => setEditingId(null)} className="px-3 py-1.5 text-xs font-bold text-slate-500 hover:text-blue-600 uppercase">Cancel</button>
                                ...
                                <div className="relative group/mobile-menu">
                                    ...
                                    <div className="absolute right-0 top-full mt-1 w-32 bg-white shadow-sm border border-slate-200 border border-slate-300 rounded-lg shadow-xl overflow-hidden z-20 hidden group-hover/mobile-menu:block">
                                        <div className="p-1 space-y-0.5">
                                            ...
                                            <button onClick={(e) => { e.stopPropagation(); startEdit(task); }} className="w-full text-left px-2 py-1 text-[10px] text-blue-400 hover:bg-slate-200 rounded flex items-center gap-2"><Edit2 className="w-3 h-3" /> Edit</button>
                                ...
                                <div className="relative group/menu">
                                    ...
                                    <div className="absolute right-0 top-full mt-1 w-32 bg-white shadow-sm border border-slate-200 border border-slate-300 rounded-lg shadow-xl overflow-hidden z-20 hidden group-hover/menu:block hover:block">
                                        <div className="p-1 space-y-0.5">
                                            ...
                                            <div className="h-px bg-slate-100 border border-slate-200 my-1" />
                                            <button onClick={() => startEdit(task)} className="w-full text-left px-2 py-1 text-[10px] hover:bg-slate-200 text-blue-400 hover:text-blue-300 rounded flex items-center gap-2"><Edit2 className="w-3 h-3" /> Edit</button>
